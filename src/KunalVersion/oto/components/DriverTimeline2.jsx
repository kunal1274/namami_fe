...
/**
 * Displays a timeline for drivers with their availability for a given date.
 * Includes responsiveness, tooltips, sorting, and filtering.
 */
...
function DriverTimeline({ drivers, date = "2024-12-30" }) {
  ...
  const availabilityMap = useMemo(() => {
    return drivers.map((driver) => ({
      ...
      hours: hours.map((hour) => ({
        available: isAvailableThisHour(driver, hour),
  ...
  const [filterAvailableOnly, setFilterAvailableOnly] = React.useState(false);
  const [sortBy, setSortBy] = React.useState("name"); // Options: 'name', 'availability'
